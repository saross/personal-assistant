...
| `ZOTERO_API_KEY_SDAM_AU` | Personal-library write; **group write on `all`** plus explicit write on `SDAM-AU` (2366083); all-groups read. **Caveat (2026-08-21): key scope ≠ effective permission — item updates in groups `Archaeology-reproducibility` (5396607) and `HAVI application` (5940452) returned 403 `UserNotAuthorisedError` despite the `all` write scope; the user's per-group role gates writes. Verified working via their dedicated keys: FAIMS-Project (2542876), group 5861859.** | **broad** | no script yet |

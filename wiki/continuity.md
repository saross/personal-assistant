...
| Phase 5 future refinement — 8-metric gate tolerance review | **pending (deferred 2026-05-30)** — the `--validate` gate-calibration block found **0/18 corpus papers pass all 8 checks (median 4/8)**: the inherited Appendix E tolerances on **em-dash (1/18 pass), semicolon (3/18), announcement-colon (3/18) and hedge (5/18)** are tighter than the corpus's own between-paper variance, so a real corpus paper routinely fails them. The em-dash two-sided band (0.572 ±0.20) is especially ill-fitting for a bimodal metric where 12/18 papers sit at exactly 0. **Refinement:** re-derive those four tolerances from the per-paper spread now quantified in `phase5-validation-report.md` (e.g. target = aggregate ± between-paper-stdev, or a CV-aware band), reconsider whether the bimodal em-dash check should be a one-sided ceiling rather than a band, and decide whether `--modern-em-dash` should become the default for new-prose evaluation. Until then, the evaluator faithfully implements Appendix E and the report documents that a FAIL on those four is a *deviation flag*, not proof of off-voice text — cross-check the Mahalanobis distance. Reference: `data/style-corpus/phase5-validation-report.md` gate-calibration section; plan §2.4 / guide Appendix E |

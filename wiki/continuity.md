...
### 2026-05-31 (Sun, latest PA) — Items 20 + 21 (a+b) shipped: `verify_file` hardening, file-gate tightening, prefix-recovery diagnostic; `verified=false` is now legible (genuinely-suspect set is tiny)
...
- **GATED follow-up (Shawn's call):** rewriting the 118 recoverable refs in `memories.jsonl` is a corpus mutation needing sign-off + a quiet window (daily-sync owns the file). Diagnostic in place to drive it when approved.

...
### G. Style-guide construction (multi-genre, academic kick-off) — *v2 Phases 1 + 2 + 3 + 4 done 2026-05-30 (Phase 1 legacy+clean+Stream A; Phase 2 Biber; Phase 3 Kumar+bimodality+verifier; Phase 4 Panickssery); v2.3 agent ready; Phase 5 (Mahalanobis evaluator) only remaining workstream-G item*
...
| Phase 5 — Mahalanobis evaluator | **scoped, separate downstream tool** — 3–5 h estimate; build `scripts/style-analyser/phase5_evaluator.py` consuming `phase1-results-clean.json`; per-paper feature vectors → Mahalanobis distance with Ledoit-Wolf shrinkage (18 samples × ~10 features means raw covariance overfits); leave-one-paper-out validation; +8-metric tolerance gate (already in Appendix E). Self-contained, no API. To be undertaken in a fresh session per Shawn's 2026-05-30 decision (in-conversation context cost vs fresh-session re-orientation cost favours fresh) |

...
- **Working-notes candidates HELD OVER (no verdict yet — no-silent-discard):** (A) `community.cosmos-institute.org/api/search` is an undocumented clean JSON API — check for a data backend before browser automation on "not machine-optimised" pages; (B) session-archaeology workflow validated (PG sessions-table title search → archived-transcript zgrep → subagent transcripts → Gmail triangulation, ~15 min to full recovery) + the hazard it surfaced (local-only repos on a multi-machine setup are invisible from other hosts).
- **Also still pending:** the 2026-07-05 PA-infra user-obs candidates (in `wiki/user-observations.md`) never got verdicts; today's 4 candidates join them.
- **Loose ends handed to next session/standup:** Monday's session time untracked (suggest `/track career ~2h` ARDC prep); `tasks/waiting-for.md` ARDC row predates the funded-bid news + booked meeting — update at standup; Cosmos first move (open Airtable form, capture questions) is the Slot 2 task, clock running since Mon 6 Jul.

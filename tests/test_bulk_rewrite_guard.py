...
def _staged_copy(tmp_path: Path) -> tuple[Path, Path, Path]:
    ...
    Returns ``(scripts_dir, expected_log_dir, expected_log_file)``. The log
    directory is deliberately NOT created: its appearance is one of the
    things the tests assert against.
    """
    repo = tmp_path / "repo"
    ...
    log_dir = repo / "logs"
    return scripts, log_dir, log_dir / "bulk-rewrite-guard.log"


def _run_child(program: str, scripts: Path, home: Path) -> dict:
    """Run *program* in a fresh interpreter and return its JSON report."""
...
class TestImportSideEffects:
    ...
    def test_import_under_pytest_creates_no_log(self, tmp_path):
        ...
        report = _run_child(program, scripts, tmp_path / "home")

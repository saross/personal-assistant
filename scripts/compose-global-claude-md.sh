...
#
# Layering (GPT-integration plan §6, Phase 2):
#   1. global-agent-guidance/common.md   — portable, agent-neutral guidance.
#      SHARED editing surface: Claude and GPT may both propose changes to it,
#      and GPT's gpt-hub installer composes the same file into
#      ~/.codex/AGENTS.md. Nothing harness-specific belongs here.
#   2. global-claude-md/claude.md        — Claude-owned overlay: commands,
#      tools, models, and Claude-owned stores. Never read by GPT's composer.
...
# ~/.codex/AGENTS.md, and GPT's installer must never write ~/.claude/CLAUDE.md.
#
# Usage:
